class ApplicationController < ActionController::Base
  ...
  before_action :configure_permitted_parameters, if: :devise_controller?
	...
	def configure_permitted_parameters
      ...
      def move_temp_to_current    # moves the items from temporary cart to the currently used cart
          old_cart = session[:temporary_cart]
          old_cart.each do |cartItem|
            item_id = cartItem[0].to_i
            item = Item.find(item_id)
            cart_item = current_cart.cart_items.create(item: item)
